...
class TestAgentThoughtsCallback:
    ...
    def test_should_log_agent_thoughts_when_agent_responds(self):
        ...
        callback_context.agent.name = "CoordinatorAgent"
        callback_context.agent._logger = mock_logger
        callback_context.llm_response.text = "I need to gather oak logs first, then craft planks."
        callback_context.llm_response.candidates = []

        # Act
        result = log_agent_thoughts_callback(callback_context)
    ...
    def test_should_log_tool_calls_when_agent_uses_tools(self):
        ...
        callback_context = Mock()
        callback_context.agent.name = "GathererAgent"
        callback_context.agent._logger = mock_logger
        callback_context.llm_response.text = None
        callback_context.llm_response.candidates = [Mock(content=Mock(parts=[part]))]
        ...
        assert call_args[1]["agent"] == "GathererAgent"
...
class TestToolInvocationCallbacks:
    ...
    def test_should_log_tool_start_with_context(self):
        ...
        tool_context = Mock()
        tool_context.agent.name = "CrafterAgent"
        tool_context.agent._logger = mock_logger
        tool_context.tool_name = "craft_item"
        tool_context.tool_args = {"item_name": "oak_planks", "quantity": 4}
        tool_context.session.state = {"minecraft.inventory": {"oak_log": 1}}

        # Act
        with patch("time.perf_counter", return_value=1000.0):
            result = log_tool_invocation_start_callback(tool_context)
        ...
        assert call_args[1]["agent"] == "CrafterAgent"
    ...
    def test_should_log_tool_end_with_duration(self):
        ...
        tool_context.agent._logger = mock_logger
        tool_context.tool_name = "craft_item"
        tool_context.tool_result = {"status": "success", "crafted": 4}
        tool_context._start_time = 1000.0
        tool_context.session.state = {"minecraft.inventory": {"oak_planks": 4}}

        # Act
        with patch("time.perf_counter", return_value=1001.5):
            result = log_tool_invocation_end_callback(tool_context)
