...
def create_gather_result(status: str, gathered: int = 0, item_type: str = "", error: str = "") -> dict:
    ...
    """
    result = {"status": status, "gathered": gathered, "item_type": item_type}
    if error:
        result["error"] = error
    return result


def create_craft_result(
    status: str, crafted: int = 0, item_type: str = "", missing_materials: list = None, error: str = ""
) -> dict:
    ...
    """
    result = {"status": status, "crafted": crafted, "item_type": item_type}
